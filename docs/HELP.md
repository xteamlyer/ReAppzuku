...
Prevents the app from launching screens (Activities) and bringing them to the foreground while the app itself is in the background. This restriction protects the user from sudden pop-up windows.\
**Blocks:** unexpected opening of the app's interface (for example, ads or full-screen incoming call windows) while you are using another app or when the device is locked.\
**Does not block:** sending notifications, running background services, and launching screens if the user directly taps on a notification generated by this app.
...
| START_ACTIVITIES_FROM_BACKGROUND | — | ✓ | ✓ | optional |
| WAKE_LOCK | — | — | ✓ | optional |
