...
为了让 ReAppzuku 可靠运行而不被系统终止，请正确配置权限。具体步骤取决于您的固件。

---

### 电池优化（所有固件）

最重要的一步。如果不禁用，系统会定期终止 ReAppzuku。
...
> [!CAUTION]
> 本节内容仅供参考，并非 ReAppzuku 的使用说明。请自行谨慎配置所有功能，因为某项功能设置不当可能会影响目标应用的正常运行。

乍一看，ReAppzuku 似乎是一个设置繁多、结构复杂的工具。但实际上，理解它并不难。

**初始设置**\
授予 Root 或 Shizuku 权限。请务必解除系统对本应用的所有限制（电池优化、禁止自启动、最近任务中的锁定等）。\
启动 ReAppzuku 后，进入设置并开启**后台服务**——该设置会启动应用的后台进程：应用监控、统计数据收集等。

**统计数据收集**\
不要急于立即启用自动终止或配置其他限制。给应用 1–2 天时间收集统计数据——这样可以更准确地确定哪些应用真正在后台占用设备资源。

**数据分析**\
收集数据后，进入**"统计与日志"**部分，查看资源消耗图表（电池、CPU、内存）。你也可以对照手机设置中的电池统计数据。图表图例中可以看到哪些应用消耗的资源最多。

**配置自动终止**\
建议从**黑名单**模式开始——该模式更安全，因为只影响明确选定的应用，不会波及其他应用。自动终止的周期间隔初期可设为 1 分钟，之后再根据需要进行调整。\
自动终止运行 1–2 小时后，进入"统计与日志"部分，打开 **Top Offenders** 日志。你会看到哪些应用最常被终止，以及其中哪些会立即重启。\
重启次数超过 3 次的应用是**后台限制**的良好候选对象。可以先为它们设置 **Soft** 类型的后台限制。

**精细化配置**\
如需更深入的分析，可使用**主界面**上的**应用触发器**按钮——它会显示应用具体使用了哪些方式在后台运行。根据结果，可以应用其他类型的后台限制。更多详情请参阅指南相应章节。

> [!TIP]
> 自动终止只是消除了应用在后台运行这一事实，但并未消除它返回后台的原因。若要实现全面控制，建议同时配置**后台限制**和**休眠模式**。
...
|---|---|
| **终止应用** | 终止当前前台应用 |
| **终止后台应用** | 根据您的白名单/黑名单设置运行自动终止 |

**小部件**\
主屏幕小部件——显示最近 12 小时的 自动终止统计数据和当前 RAM 负载。

**快捷方式**\
通过长按应用图标使用静态快捷方式——终止当前前台应用。
...
通过系统命令实时分析 **63 个独立因素（43 个主要因素和 20 个取决于 Android 版本的附加因素）**。
...
> - 41–65——中等水平。自动终止 或 Soft 类型的后台限制可能已足够。
> - 66+——自动终止、Hard 或 Manual 类型的后台限制，或休眠模式的理想候选。
...
- **Foreground Service**。
应用启动了一个带有持久通知的后台服务。这是避免被终止的最可靠方式——只要通知可见，Android 就不会触碰此类进程。显示服务类型：媒体播放、位置、通话、已连接设备等。

- **FG 通知渠道**。
补充 Foreground Service 信息：显示通知渠道的重要性。URGENT 或 HIGH 重要性以弹出横幅形式显示——系统极难抑制，使 force-stop 几乎不可能。

- **Sticky Service**。
服务声明了 `START_STICKY`——Android 在终止后会自动重启它。不禁用该应用就无法永久停止。

- **被绑定持有**。
一个或多个进程持有对此应用服务的活跃绑定。只要绑定存在，Android 就无法终止进程。Google Play Services (GMS) 是常见的罪魁祸首——持有推送连接和账户同步绑定。
...
- **广播效率**。
显示有多少广播被传递给应用，以及有多少需要冷启动。高百分比 = 系统定期终止并重启它。
...
- ContentProvider。
应用声明了一个或多个 ContentProvider。其他应用或系统可通过 URI 直接访问它们——当收到请求时，Android 会自动启动该进程（即使它之前未在运行）。显示已注册 provider 的 authority 地址。
...
- **进程冻结**。
Android 11–13：进程被系统通过 cgroup freezer 冻结——执行暂停，但未被终止。在访问时自动解冻。
...
**锁屏时终止**\
在屏幕锁定的瞬间运行终止。适合每次放下手机时进行清理。

**RAM 负载时终止**\
附加条件——终止仅在 RAM 超过选定阈值 **时** 触发。适用于定期终止和锁屏终止。
...
- **🛡️ 白名单**——终止所有后台应用，**白名单中的除外**。用于最大化清理。
> [!WARNING]
> 使用白名单时，强烈建议将大多数系统应用添加到白名单中，以避免设备出现问题

- **🎯 黑名单（默认）**——**仅**终止黑名单中的应用。用于停止特定应用而不影响其他应用。

**白名单 / 黑名单**\
适用于所选模式的应用列表。根据模式显示两个列表之一。

**高级条件**\
使用额外触发器扩展自动终止——适用于常规计划不够用的情况。

- **硬件事件**。
自动终止在选定事件发生时自动启动：耳机或 USB 连接/断开、充电状态变化、WiFi、移动网络、蓝牙、GPS 或热点。事件发生后，保持 10 秒暂停——以便寄生应用有时间启动并被清理。

- **应用启动**。
自动终止在选定的目标应用被打开时立即触发——在预算设备上，有助于在启动大型游戏或程序之前释放 RAM 和 CPU。目标应用本身不会被终止。
  - **清除缓存**。额外清除所有应用的缓存，受保护应用、持久应用和其他目标应用除外。
> [!IMPORTANT]
> **应用启动** 功能需要在"无障碍"设置中授予特殊权限。此功能也可能略微增加 ReAppzuku 自身的电池使用量。

**自动终止预设**\
保存您自己的自动终止设置集，在一天中的特定时间自动激活，并在其活跃窗口期间替换当前设置。窗口结束后，原始设置会自动恢复。
提供 **2 个预设**。每个可独立配置：自己的名称、自己的活跃时间范围、自己的自动终止规则、自己的应用列表以及自己的附加场景。
> [!WARNING]
...
- **自动终止管理和高级条件**。
标准的 自动终止设置块，与常规应用设置中相同。所有这些设置在 [自动终止设置](#-auto-kill-settings) 中有详细描述。
...
点击快捷方式会根据当前设置触发即时自动终止并清除 RAM。

> [!TIP]
> 无论自动终止期间是否关闭了应用，RAM 都会被清除。使用 am send-trim-memory 命令清除 RAM。仅白名单和应用持久进程不受影响。
...
使用 Android 的 `appops` 在系统层面 **阻止应用在后台运行**。比常规终止更深入。
...
阻止任何后台活动。\
**工作原理：**\
一旦应用被最小化或切换离开——系统立即终止它。应用无法在没有直接用户交互的情况下保持在内存中（即使可见于最近任务中）。谨慎使用 Hard 限制，因为它可能完全剥夺应用的后台操作（文件下载、媒体播放、长时间运行的内部任务）。\
**使用的命令：**\
`RUN_ANY_IN_BACKGROUND ignore`\
`RUN_IN_BACKGROUND ignore`\
...
**隐藏应用**\
此处的应用不会出现在主屏幕上，也永远不会被自动终止触及。适用于您不需要查看的服务进程。
...
**自动终止日志**\
显示最近 **12 小时** 的活动：自动终止次数、重启次数、释放的 RAM 以及每个应用的最后事件时间。

> [!TIP]
> 重启超过 3 次的应用是后台限制的良好候选。

**Top Offenders**\
按综合评分（终止次数 + 重启次数 + RAM 使用量）对应用进行排名。筛选条件：12 小时 / 24 小时 / 7 天 / 全部时间。

> [!NOTE]
> 评分显示应用对后台管理的干扰程度。\
>
> `评分 = 终止次数 × 1 + 重启次数 × 2 + 释放的 RAM × 0.01`
>
> • 终止 (+1)——应用被 force-stop。\
...
> [!IMPORTANT]
> 释放的 RAM 仅在下次自动终止周期中发现应用未运行时才被计入。如果它重启了，它会重新占用相同的 RAM——净收益为 0%。
...
这些应用 **永远不会受到**自动终止或其他限制的影响，无论设置如何：
...
不需要。主屏幕上的手动终止、快捷磁贴、小部件和快捷方式均可在没有后台服务的情况下工作。
...
**❓ 为什么将终止类型从 force-stop 改为 am kill？**

`am force-stop` 是硬性停止——终止所有进程并清除应用状态。`am kill` 更温和——仅终止后台进程而不触及前台。仅当您发现其他应用出现问题或固件冲突时才切换——在某些设备上 `force-stop` 过于激进。
